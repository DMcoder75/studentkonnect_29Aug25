...
const HeaderWithDropdowns = () => {
  ...
  return (
    <>
      ...
      <header className="bg-white shadow-sm border-b border-gray-200 py-2 sm:py-4 px-2 sm:px-4">
        <div className="container mx-auto flex items-center justify-between">
          {/* Logo */}
          <div 
            onClick={() => navigate('/')}
            className="desktop-header-logo flex items-center space-x-2 sm:space-x-3 cursor-pointer"
          >
            <div className="w-8 h-8 sm:w-12 sm:h-12 bg-gradient-to-br from-purple-600 to-blue-600 rounded-xl flex items-center justify-center shadow-lg">
              <span className="text-white font-bold text-sm sm:text-lg">SK</span>
            </div>
            <span className="logo-text text-lg sm:text-2xl font-bold text-gray-800 hidden sm:block">StudentKonnect</span>
            <span className="logo-text-mobile text-sm font-bold text-gray-800 sm:hidden">SK</span>
          </div>

          {/* Search Bar - Hidden on mobile */}
          <div className="desktop-header-search hidden md:flex flex-1 max-w-2xl mx-8">
          ...
          {/* Action Buttons */}
          <div className="desktop-header-buttons flex items-center space-x-1 sm:space-x-4">
            {user ? (
              // Logged in user menu - Simplified for mobile
              <div className="relative">
                <button 
                  onClick={() => toggleDropdown('userMenu')}
                  className="flex items-center space-x-1 sm:space-x-2 text-purple-600 hover:text-purple-700 font-medium px-2 sm:px-4 py-1 sm:py-2 rounded-lg hover:bg-purple-50 transition-all duration-300"
                >
                  <div className="w-6 h-6 sm:w-8 sm:h-8 bg-gradient-to-br from-purple-400 to-blue-500 rounded-full flex items-center justify-center text-white font-semibold text-xs sm:text-sm">
                    {user.full_name ? user.full_name.charAt(0) : (user.firstName ? user.firstName.charAt(0) : 'U')}
                  </div>
                  <span className="hidden sm:inline">Hi, {user.full_name || user.firstName || user.name}</span>
                  <ChevronDown className={`h-3 w-3 sm:h-4 sm:w-4 transition-transform duration-200 ${dropdownStates.userMenu ? 'rotate-180' : ''}`} />
                </button>
              ...
              // Not logged in - show sign in button - Mobile responsive
              <button 
            ...
            <button 
              ...
              className="flex items-center space-x-1 sm:space-x-2 bg-purple-600 text-white px-2 sm:px-4 py-1 sm:py-2 rounded-lg hover:bg-purple-700 transition-colors shadow-md"
            >
